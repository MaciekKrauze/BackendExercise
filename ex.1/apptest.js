...
class Book {
    ...
    borrow() {
        if (!this.isAvailable) throw new Error('Brak dostępnych egzemplarzy');
        this.borrowedCopies++;
    }

    return() {
        if (this.borrowedCopies === 0) throw new Error('Nie można zwrócić');
        this.borrowedCopies--;
    }
    ...
    static isValidBook(bookData) {
        return Validator.isValidISBN(bookData.isbn) && Validator.isValidYear(bookData.publicationYear) && bookData.totalCopies > 0;
    }
...
User.prototype.hasOverdueBooks = function(days) {
    return this.borrowHistory.some(h => !h.returned && DateUtils.getDaysBetween(h.borrowDate, new Date()) > days);
};
